#!/usr/bin/env python3
"""HFT orderbook collector for Kalshi: subscribe via WebSocket, store snapshot + frames/deltas + optional trades."""
    ...
    def _on_trade(event):
        ...
        append_trade_line(
            ...
            mid,
            {
                ...
                "kalshi_msg": (event.data or {}).get("kalshi_msg"),
            },
        )
...
async def run(
    ...
    *,
    ...
    for mid in market_ids:
        await ws.subscribe_market(mid, subscribe_orderbook=True, subscribe_trades=include_trades)
    logger.info(
        "Subscribed to %d Kalshi markets (trades=%s)",
        len(market_ids),
        "on" if include_trades else "off",
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="HFT orderbook collector for Kalshi (snapshot + 1s frames or deltas; optional public trades WS)"
    )
    ...
    parser.add_argument(
        "--no-trades",
        action="store_true",
        help="Do not subscribe to Kalshi public trade channel (default: trades on, written to trades.jsonl)",
    )
    parser.add_argument(
        "--mode",
        type=str,

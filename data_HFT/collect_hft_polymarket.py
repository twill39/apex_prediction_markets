#!/usr/bin/env python3
"""HFT orderbook collector for Polymarket: subscribe via WebSocket, store initial snapshot + 1s frames/deltas."""
...
async def run(output_root: str, interval: float, duration: float, market_ids: list, mode: str) -> None:
    ...
    logger.info("Subscribed to %d Polymarket assets", len(market_ids))
...
def main():
    parser = argparse.ArgumentParser(
        description="HFT orderbook collector for Polymarket (snapshot + 1s frames or deltas)"
    )
...
#!/usr/bin/env python3
"""HFT orderbook collector for Polymarket: subscribe via WebSocket, store initial snapshot + 1s deltas."""
    ...
    while ws.is_running and ws.is_connected:
        ...
        for mid in market_ids:
            ...
            if curr is None:
                continue
            if not snapshot_written.get(mid):
                if curr.get("bids") or curr.get("asks"):
                    ts_iso = datetime.now(timezone.utc).isoformat()
                    write_snapshot(output_root, platform, mid, curr, ts_iso)
                    snapshot_written[mid] = True
                    last_emitted_books[mid] = copy.deepcopy(curr)
